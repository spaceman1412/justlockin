...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  private func showContextMenu() {
    ...
    if viewModel.currentSessionType == .overflow {
      ...
      // Show rewind option only if there's history, or reset option if timer is not idle
      if viewModel.timerState == .idle {
        // When idle, only show if there's previous session history
        rewindMenuItem.isEnabled = viewModel.canRewindToPreviousSession
      } else {
        // When running/paused, always allow reset
        rewindMenuItem.isEnabled = true
      }
